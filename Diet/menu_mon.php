...
$monc_s1br = mysqli_query($conn, "SELECT item.Item_name, mon_breakfast.C_S1, item.unit FROM item INNER JOIN mon_breakfast ON item.Item_id = mon_breakfast.Item_id WHERE C_S1 <> 0.000");
$monc_s1lu  = mysqli_query($conn, "SELECT item.Item_name, mon_lunch.C_S1, item.unit FROM item INNER JOIN mon_lunch ON item.Item_id = mon_lunch.Item_id WHERE C_S1 <> 0.000");
$monc_s1di  = mysqli_query($conn, "SELECT item.Item_name, mon_dinner.C_S1, item.unit FROM item INNER JOIN mon_dinner ON item.Item_id = mon_dinner.Item_id WHERE C_S1 <> 0.000");

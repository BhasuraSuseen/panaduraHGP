...
$monc_s1lu  = mysqli_query($conn, "SELECT item.Item_name, menu_wed.C_S1Lu, item.unit FROM item INNER JOIN menu_mon ON item.Item_id = menu_mon.Item_id WHERE C_S1Lu <> 0.000");
$monc_s1di  = mysqli_query($conn, "SELECT item.Item_name, menu_wed.C_S1Di, item.unit FROM item INNER JOIN menu_mon ON item.Item_id = menu_mon.Item_id WHERE C_S1Di <> 0.000");
